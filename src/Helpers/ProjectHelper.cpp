...
std::vector<char> ProjectHelper::ReadFile(const std::string& filename)
{
...
ProjectHelper::SwapChainSupportDetails ProjectHelper::QuerySwapChainSupportDetail(VkPhysicalDevice physicalDevice, const VkSurfaceKHR& windowSurface)
{
...
void ProjectHelper::CopyBufferToBuffer(const VkQueue queue, const VkCommandPool& cmdPool, const VkBuffer& src, VkDeviceSize srcOffset, const VkBuffer& dst, VkDeviceSize dstOffset, VkDeviceSize size)
{
...
bool ProjectHelper::IsFormatSupported(VkFormat format, VkImageTiling tiling, VkFormatFeatureFlags features)
{
...
VkImageAspectFlags ProjectHelper::GetImageAspectFlags(VkFormat format)
{
    switch(format)
    {
...
bool ProjectHelper::HasStencilComponent(VkFormat format)
{
    return format == VK_FORMAT_S8_UINT || format == VK_FORMAT_D32_SFLOAT_S8_UINT || format == VK_FORMAT_D24_UNORM_S8_UINT;
}

uint32_t ProjectHelper::GetFormatSize(VkFormat format)
{
    switch(format)
    {
        case VK_FORMAT_D16_UNORM:
...
glm::quat ProjectHelper::EulerToQuat(glm::vec3 euler)
{
